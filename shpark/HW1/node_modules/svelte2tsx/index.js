...
function convertHtmlxToJsx(str, ast, onWalk = null, onLeave = null) {
    ...
    const handleBinding = (attr, el) => {
        const getThisTypeForComponent = (node) => {
        ...
        if (attr.name == 'this' && supportsBindThis.includes(el.type)) {
            ...
            if (thisType) {
                str.remove(attr.start, attr.expression.start);
                str.appendLeft(attr.expression.start, '{...__sveltets_empty(');
                str.overwrite(attr.expression.end, attr.end, `=__sveltets_instanceOf(${thisType}))}`);
                return;
            }
        }
        //one way binding
    ...
    const handleIdentifier = (node, parent, prop) => {
        ...
        if (node.name[0] == '$') {
            if (isDeclaration) {
                ...
                if (parent.type == 'MemberExpression' && prop == 'property')
                    return;
                if (parent.type == 'Property' && prop == 'key')
                    return;
...
function isTsFile(scriptTag, moduleScriptTag) {
    return tagIsLangTs(scriptTag) || tagIsLangTs(moduleScriptTag);
    function tagIsLangTs(tag) {
        var _a;
        return (_a = tag === null || tag === void 0 ? void 0 : tag.attributes) === null || _a === void 0 ? void 0 : _a.some((attr) => {
            var _a;
            if (attr.name !== 'lang' && attr.name !== 'type') {
                return false;
            }
            const type = (_a = attr.value[0]) === null || _a === void 0 ? void 0 : _a.raw;
            switch (type) {
                case 'ts':
                case 'typescript':
                case 'text/ts':
...
function createRenderFunction({ str, scriptTag, scriptDestination, slots, getters, exportedNames, uses$$props, uses$$restProps }) {
    const htmlx = str.original;
    let propsDecl = '';
    if (uses$$props) {
    ...
    const slotsAsDef = '{' +
        Array.from(slots.entries())
            ...
            .join(', ') +
        '}';
    const returnString = `\nreturn { props: ${createPropsStr(exportedNames)}, slots: ${slotsAsDef}, getters: ${createRenderFunctionGetterStr(getters)} }}`;
    str.append(returnString);
}
function createPropsStr(exportedNames) {
...
function svelte2tsx(svelte, options) {
    ...
    let { moduleScriptTag, scriptTag, slots, uses$$props, uses$$restProps, componentDocumentation, } = processSvelteTemplate(str);
    ...
    addComponentExport(str, uses$$props || uses$$restProps, !!(options === null || options === void 0 ? void 0 : options.strictMode), isTsFile(scriptTag, moduleScriptTag), getters, className, componentDocumentation);
    return {
        code: str.toString(),
